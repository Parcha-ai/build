...
for (const bundledAdhdFile of ['SKILL.md', 'LICENSE', path.join('agents', 'openai.yaml')]) {
  ...
  if (!fs.existsSync(bundledAdhdPath)) {
    fail(`packaged ADHD-friendly output resource missing at ${bundledAdhdPath}`);
  }
  console.log(`ok packaged ADHD-friendly output resource: ${bundledAdhdPath}`);
}
...
const mainMarkers = [
  ...
  'Foreground SSH cleanup failed',
  'Stream-end SSH cleanup scheduled in background',
  'Resume SSH cleanup scheduled in background',
  'Deferring STREAM_END for recoverable SSH turn',
  'Remote turn survived or may have survived local stream error',
  'SSH reattach unavailable for',
  ...
  'preserving the remote turn, requesting reattach',
  ...
  'Planning harness stopped',
  're-entering Auto Build for execution',
