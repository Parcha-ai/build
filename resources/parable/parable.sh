...
build_proxy=false
for argument in "$@"; do
  [[ "$argument" == "--build-proxy" ]] && build_proxy=true
done
if [[ "$build_proxy" == true ]] && ! command -v go >/dev/null 2>&1; then
  if command -v brew >/dev/null 2>&1; then
    printf 'parable prerequisite: installing Go with Homebrew for the pinned proxy build\n'
    HOMEBREW_NO_AUTO_UPDATE=1 brew install go
  else
    die "Go is required to build the pinned proxy; install Go and retry"
  fi
fi
