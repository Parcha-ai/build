...
function buildProxy(options, log) {
  ...
  const existingDestination = lstatOrNull(destination);
  if (existingDestination) {
    const binary = path.join(destination, PROXY_BINARY_NAME);
    const binaryStat = lstatOrNull(binary);
    const baseRevision = runChecked(
      git, ["-C", destination, "rev-parse", "HEAD~2"], { capture: true },
    ).stdout.trim();
    const trackedChanges = runChecked(
      git, ["-C", destination, "status", "--porcelain", "--untracked-files=no"], { capture: true },
    ).stdout.trim();
    if (
      existingDestination.isSymbolicLink()
      || !existingDestination.isDirectory()
      || modeOf(existingDestination) !== 0o700
      || !binaryStat
      || binaryStat.isSymbolicLink()
      || !binaryStat.isFile()
      || (binaryStat.mode & 0o111) === 0
      || baseRevision !== PROXY_COMMIT
      || trackedChanges
    ) {
      throw new OnboardingError(`existing build destination is not a verified pinned build: ${destination}`);
    }
    log(`reusing pinned CLIProxyAPI -> ${binary}`);
    return binary;
  }
